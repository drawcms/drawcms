...
and MP4 export are locked there by default (point `NEXT_PUBLIC_CLOUD_URL` at a
managed instance to unlock them); where MP4 is enabled, video export still
degrades at runtime — the Export menu disables MP4 where WebCodecs is missing.
