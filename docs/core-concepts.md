...
required. SVG and WebM/MP4 recordings are DrawCMS Cloud features — the export
menu shows them as locked with a link to Cloud. Sharing (view-only links and
embeds) is also Cloud-only; the self-hosted editor has no built-in sharing.
