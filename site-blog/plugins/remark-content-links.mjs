...
export function remarkContentLinks(options) {
  const {
    githubBase = "https://github.com/drawcms/drawcms",
    ref = "main",
    contentRoot,
    // URL prefix for docs pages — e.g. "docs/" when the docs site serves at
    // drawcms.com/docs. Empty string keeps the legacy domain-root layout.
    docsPrefix = "",
  } = options;
    ...
    visit(tree, "link", (node) => {
      ...
      if (repoPath.startsWith("docs/")) {
        ...
        if (/\.(md|mdx)$/.test(docsPath)) {
          node.url = toSlug(docsPath, docsPrefix) + hash;
        } else {
          node.url = `${githubBase}/blob/${ref}/${repoPath}`;
        }
        return;
