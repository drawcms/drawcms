...
describe("public API surface", () => {
  ...
  // Importing the barrel transforms the entire editor module graph, which is the
  // point of the test — it is the one place a regressed export is caught. That
  // makes it the slowest import in the suite, and it grows with the editor, so it
  // needs a timeout well above the 5s default: under full parallel load it went
  // from ~1s standalone to over 5s once WebMCP composition landed. A longer
  // budget here is honest; trimming the assertions would not be.
  it(
    "exposes document, command, plugin, and persistence building blocks",
    { timeout: 30_000 },
    async () => {
